...
public class ScrollViewGestureHandler: NSObject, UIGestureRecognizerDelegate {
    ...
    @objc func handlePanGesture(_ gesture: UIPanGestureRecognizer) {
        guard let scrollView = scrollView,
              let resizableSheetModel = resizableSheetModel else {
                  return
              }
        ...
        case .changed:
            let diff = -gesture.translation(in: view).y + startOffset
            
            if (
                resizableSheetModel.state != .large
                && resizableSheetModel.mediumSize.height < resizableSheetModel.fullSize.height
            )
                || scrollView.contentOffset.y < 0
                || state == .changed
            {
                state = .changed
                resizableSheetModel.updateOffSet(diff: diff)
